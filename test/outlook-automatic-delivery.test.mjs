...
const { outlookFailureMessage } = await import('../src/domain/outlook/outlookFailures.js');
...
test('deneme eşiği mesajı otomatik duruşu ve kullanıcı işlemini açıklar', () => {
  const message = outlookFailureMessage('OUTLOOK_RETRY_EXHAUSTED');
  assert.match(message, /otomatik yeniden deneme durdu/);
  assert.match(message, /Yeniden ekleme, yeniden gönderme veya kaldırma/);
  assert.doesNotMatch(message, /sürüyor/);
});

test('tamamlanmış abonelik boş yere taranmaz; yeniden açılınca aynı UID ile teslim edilir', async (t) => {
  ...
  await enqueueOutlookTaskChange(pool, TASK, before, db.tasks[0]);
  await advance(5000);
  assert.equal(row.CompletionSuspended, 1);
  assert.equal(row.PendingMethod, null);
  assert.equal(row.DeliveredMethod, 'CANCEL');
  const queueSeq = row.QueueSeq;
  const validatedAt = row.LastValidatedAt;
  await advance(600000);
  assert.equal(value.status().lastResult.claimed, 0);
  assert.equal(row.QueueSeq, queueSeq);
  assert.equal(row.LastValidatedAt, validatedAt);
  assert.equal(sent.length, 2);
  const completed = { ...db.tasks[0] };
  db.tasks[0].Status = 'in-progress';
  await enqueueOutlookTaskChange(pool, TASK, completed, db.tasks[0]);
  await advance(5000);
  assert.equal(row.CompletionSuspended, 0);
  assert.deepEqual(sent.map((message) => fields(message).method), ['REQUEST', 'CANCEL', 'REQUEST']);
  assert.deepEqual(sent.map((message) => fields(message).sequence), [1, 2, 3]);
...
test('deneme eşiğindeki kuyruk yalnız açık kullanıcı işlemiyle yeniden denenir', async (t) => {
  const { db, pool, worker, sent, advance } = await stack(t);
  await service.addTaskToOutlook(pool, { taskId: TASK, sicil: SICIL, queueOnly: true });
  const row = db.taskOutlookSubscriptions[0];
  row.AttemptCount = 6;
  row.LastFailureCode = 'SMTP_SEND_FAILED';
  const uid = row.CalendarUid;
  const value = worker();
  await advance();
  ...
  assert.equal(row.LastFailureCode, 'SMTP_SEND_FAILED');
  assert.equal(value.status().lastResult.exhausted, 1);
  assert.equal(value.status().lastResult.ok, false);
  await service.addTaskToOutlook(pool, { taskId: TASK, sicil: SICIL, queueOnly: true });
  assert.equal(row.AttemptCount, 0);
  await advance(5000);
  assert.equal(sent.length, 1);
...
test('deneme eşiğindeki yeniden gönderme açık eylemle toparlanır; bekleyen tıklamalar birleşir', async (t) => {
  const { db, pool, worker, sent, advance } = await stack(t);
  await service.addTaskToOutlook(pool, { taskId: TASK, sicil: SICIL, queueOnly: true });
  worker();
  await advance();
  ...
  assert.equal(row.NextAttemptAt, null);
  assert.equal(row.LastFailureCode, null);
  assert.equal(row.QueueSeq, queueSeq + 1);
  await advance(5000);
  assert.equal(sent.length, 2);
  assert.deepEqual(fields(sent[0]), fields(sent[1]));
  assert.equal(row.PendingMethod, null);

...
test('farklı uygulama örnekleri aynı SQL yönetim kilidini paylaşır', async (t) => {
  const db = stack(t);
  const otherDriver = createFakeSqlServerDriver(db);
  const otherPool = await new otherDriver.ConnectionPool({ database: 'MERGEN_Rota' }).connect();
  const transaction = new otherDriver.Transaction(otherPool);
  await transaction.begin();
  const lock = transaction.request();
  lock.input('resource', 'mergen-rota:admin:shared-action');
  await lock.query('EXEC sys.sp_getapplock');
  let calls = 0;
  try {
    await assert.rejects(withAdminActionLock('shared-action', () => { calls += 1; }), (error) => error.code === 'CONFLICT');
    assert.equal(calls, 0);
  } finally { await transaction.commit(); }
  await assert.rejects(withAdminActionLock('shared-action', () => { throw new Error('action-failed'); }), /action-failed/);
  assert.equal(db.adminActionLocks.size, 0);
  assert.equal(await withAdminActionLock('shared-action', () => 'done'), 'done');
  assert.equal(db.adminActionLocks.size, 0);
});

test('son otomatik hatırlatma on yeni elle gönderimin arkasında kaybolmaz', async (t) => {
  const createdAt = new Date().toISOString();
  stack(t, { reminderLog: [
    { TaskReminderLogId: 1, TaskId: TASK, ReminderKind: 'AUTOMATIC', Status: 'SENT', CreatedAt: createdAt, CompletedAt: createdAt },
    ...Array.from({ length: 12 }, (_, i) => ({ TaskReminderLogId: i + 2, TaskId: TASK, ReminderKind: 'MANUAL', Status: 'SENT', CreatedAt: createdAt }))
  ] });
  const response = await queuesRoute.GET();
  assert.equal(response.status, 200);
  const body = await response.json();
  assert.equal(body.reminders.history.length, 10);
  assert.equal(body.reminders.lastRun.kind, 'AUTOMATIC');
  assert.equal(body.reminders.lastRun.completedAt, createdAt);
});

test('eski başarılı hatırlatma kaydı süresiz sağlıklı kalmaz', async (t) => {
  const now = Date.now();
  const db = stack(t, { reminderLog: [{ ReminderKind: 'AUTOMATIC', Status: 'SENT', CreatedAt: new Date(now).toISOString(), CompletedAt: new Date(now).toISOString() }] });
  const { probeReminders } = await import('../src/server/observability/healthProbes.js');
  const { saveReminderSettings } = await import('../src/server/reminders/reminderStore.js');
  const pool = await poolApi.getSqlPool();
  await saveReminderSettings(pool, SICIL, { automaticEnabled: true, frequencyValue: 1, frequencyUnit: 'hour' });
  const fresh = await probeReminders(pool, { now });
  assert.equal(fresh.state, HEALTH_STATES.HEALTHY);
  const stale = await probeReminders(pool, { now: now + fresh.detail.staleAfterMs + 1 });
  assert.equal(stale.state, HEALTH_STATES.UNKNOWN);
  assert.equal(stale.lastSuccessAt, db.taskReminderLog[0].CompletedAt);
});

test('SMTP ve kimlik sağlığı son bağlantı testinin sonucunu ve yaşını kullanır', async (t) => {
  const sockets = new Set();
  const server = net.createServer((socket) => {
    sockets.add(socket); socket.on('close', () => sockets.delete(socket));
    socket.write('220 test SMTP\r\n');
    socket.on('data', (chunk) => {
      if (String(chunk).startsWith('EHLO')) socket.write('250 test\r\n');
      if (String(chunk).startsWith('QUIT')) socket.end('221 bye\r\n');
    });
  });
  await new Promise((resolve) => server.listen(0, '127.0.0.1', resolve));
  ...
  stack(t, { smtpHost: '127.0.0.1', smtpPort: server.address().port });
  Object.assign(process.env, {
    MERGEN_ROTA_AUTH_MODE: 'keycloak', MERGEN_ROTA_KEYCLOAK_BASE_URL: 'https://auth.test',
    MERGEN_ROTA_KEYCLOAK_REALM: 'test', MERGEN_ROTA_KEYCLOAK_CLIENT_ID: 'test',
    MERGEN_ROTA_KEYCLOAK_FLOW: 'implicit-bridge', MERGEN_ROTA_SESSION_SECRET: 'a'.repeat(40)
  });
  const previousFetch = globalThis.fetch;
  globalThis.fetch = async () => Response.json({ keys: [{ kid: 'test' }] });
  t.after(() => { globalThis.fetch = previousFetch; });
  const { probeSmtp, probeAuthentication } = await import('../src/server/observability/healthProbes.js');
  const { testIntegration, REACHABILITY_FRESHNESS_MS } = await import('../src/server/observability/integrationsService.js');
  for (const [id, probe] of [['smtp', probeSmtp], ['authentication', probeAuthentication]]) {
    assert.equal(probe().state, HEALTH_STATES.UNKNOWN);
    assert.equal((await testIntegration(null, id)).ok, true);
    const healthy = probe();
    assert.equal(healthy.state, HEALTH_STATES.HEALTHY);
    assert.ok(healthy.lastSuccessAt);
    assert.equal(probe({ now: Date.now() + REACHABILITY_FRESHNESS_MS + 1 }).state, HEALTH_STATES.UNKNOWN);
  }
  for (const socket of sockets) socket.destroy();
  await new Promise((resolve) => server.close(resolve));
  globalThis.fetch = async () => Response.json({}, { status: 503 });
  for (const [id, probe] of [['smtp', probeSmtp], ['authentication', probeAuthentication]]) {
    assert.equal((await testIntegration(null, id)).ok, false);
    assert.equal(probe().state, HEALTH_STATES.WARNING);
  }
});

test('telemetri yazma hatası uyarı ve saklama adımlarını engellemez', async (t) => {
  const db = stack(t);
  db.databaseProbeFails = true;
  const pool = await poolApi.getSqlPool();
  const now = Date.now();
  recordOperation({ operation: 'api.test', durationMs: 100, at: now - 600000 });
  queueOperationalEvent({ code: 'OPERATION_FAILED' });
  const attempted = [];
  const failing = { request() {
    ...
    request.query = async (statement) => {
      ...
      if (/MERGE dbo.MR_TelemetryOperationSamples|INSERT dbo.MR_OperationalEvents/.test(statement)) throw new Error('write-failed');
      return query(statement);
    };
    return request;
  } };
  const summary = await runTelemetryCycle({ pool: failing, now, retentionDueAt: 0 });
  assert.equal(summary.ok, false);
  assert.equal(summary.healthState, HEALTH_STATES.CRITICAL);
  assert.ok(summary.failures.some((failure) => failure.phase === 'telemetry'));
  assert.ok(summary.failures.some((failure) => failure.phase === 'events'));
  assert.equal(summary.retentionRan, true);
  assert.ok(attempted.some((statement) => statement.includes('MR_OperationalAlerts') && statement.includes('SELECT')));
  assert.ok(attempted.some((statement) => statement.includes('DELETE') && statement.includes('MR_OperationalEvents')));
});

test('başarısız tur sonucu konsola yazılır ve kritik sağlık da korunur', async (t) => {
  const { createTelemetryWorker } = await import('../src/server/observability/telemetryWorker.js');
  const logs = [];
  const worker = createTelemetryWorker({ intervalMs: 1, run: async () => ({ ok: false, reason: 'DATABASE_UNAVAILABLE', healthState: HEALTH_STATES.CRITICAL }), log: (record) => logs.push(record) });
  t.after(() => worker.stop());
  worker.start();
  await new Promise((resolve) => setTimeout(resolve, 20));
  await worker.stop();
  assert.ok(logs.some((entry) => entry.code === 'DATABASE_UNAVAILABLE'));
  assert.ok(logs.some((entry) => entry.code === 'SYSTEM_HEALTH_CRITICAL'));
});

test('birleştirilmiş olayın geri yüklemesinde bütün kayıplar sayılır', async (t) => {
  stack(t);
  const { restoreOperationalEvents } = await import('../src/server/observability/operationalEventsRepository.js');
  for (let i = 0; i < 5; i += 1) queueOperationalEvent({ code: 'OPERATION_FAILED' });
  const drained = drainOperationalEvents();
  restoreOperationalEvents(drained.events);
  queueOperationalEvent({ code: 'OPERATION_FAILED' });
  assert.equal(drainOperationalEvents().events[0].occurrenceCount, 6);
  for (let i = 0; i < 200; i += 1) queueOperationalEvent({ code: 'OPERATION_FAILED' });
  restoreOperationalEvents(drained.events);
  assert.equal(drainOperationalEvents().dropped, 5);
});

test('kayıp bildirimi dolu kuyruğa sığmazsa sayaçlar sonraki turda korunur', async (t) => {
  stack(t);
  const pool = await poolApi.getSqlPool();
  const { consumeTelemetryLossCounters } = await import('../src/server/observability/telemetryRegistry.js');
  const now = Date.now();
  for (let i = 0; i < 125; i += 1) recordOperation({ operation: `api.${i}`, durationMs: 1, at: now });
  queueOperationalEvent({ code: 'OPERATION_FAILED' });
  const saturated = { request() {
    const request = pool.request();
    const query = request.query.bind(request);
    request.query = async (statement) => {
      if (statement.includes('INSERT dbo.MR_OperationalEvents')) {
        for (let i = 0; i < 200; i += 1) queueOperationalEvent({ code: 'OPERATION_FAILED' });
      }
      return query(statement);
    };
  ...
  await runTelemetryCycle({ pool: saturated, now, retentionDueAt: Number.MAX_SAFE_INTEGER });
  assert.equal(consumeTelemetryLossCounters({ reset: false }).droppedOperations, 5);
  await runTelemetryCycle({ pool, now, retentionDueAt: Number.MAX_SAFE_INTEGER });
  assert.equal(consumeTelemetryLossCounters({ reset: false }).droppedOperations, 0);
  const loss = drainOperationalEvents().events.find((event) => event.code === 'TELEMETRY_DATA_DROPPED');
  assert.equal(JSON.parse(loss.contextJson).droppedOperations, 5);
});

test('kalıcı ölçüm sorguları aralıkla kesişen başlangıç kovasını içerir', async (t) => {
  stack(t);
  const { loadOperationSeries, loadOperationBreakdown, loadGaugeSeries } = await import('../src/server/observability/telemetryRepository.js');
  const seen = [];
  const executor = { request: () => ({ input(name, _type, value) { if (name === 'since') seen.push(value.toISOString()); return this; }, async query() { return { recordset: [] }; } }) };
  for (const load of [loadOperationSeries, loadOperationBreakdown, loadGaugeSeries]) {
    await load(executor, { since: new Date('2026-09-12T09:02:00Z'), until: new Date('2026-09-12T10:00:00Z'), bucketSeconds: 300, metricKeys: ['process.rss'] });
    await load(executor, { since: new Date('2026-09-12T09:05:00Z'), until: new Date('2026-09-12T10:00:00Z'), bucketSeconds: 300, metricKeys: ['process.rss'] });
  }
  assert.deepEqual(seen, Array.from({ length: 3 }, () => ['2026-09-12T09:00:00.000Z', '2026-09-12T09:05:00.000Z']).flat());
});

test('boş kuyruk zaman damgası SQL ikizinde 1970 olarak üretilmez', async (t) => {
  const db = stack(t, { outlookEnabled: true });
  db.taskOutlookSubscriptions.push({ PendingMethod: 'REQUEST', AttemptCount: 0, UpdatedAt: null });
  const { OUTLOOK_QUEUE_AGE_SQL } = await import('../src/server/observability/telemetryQueries.js');
  const row = (await (await poolApi.getSqlPool()).request().query(OUTLOOK_QUEUE_AGE_SQL)).recordset[0];
  assert.equal(row.OldestUnattemptedAt, null);
});

test('teslimat durum sorgusu hatası HTTP ve API hata oranına yansır', async (t) => {
  stack(t, { outlookEnabled: true });
  const pool = await poolApi.getSqlPool();
  const originalRequest = pool.request.bind(pool);
  const { OUTLOOK_QUEUE_STATUS_SQL } = await import('../src/server/outlook/outlookQueries.js');
  pool.request = () => {
    const request = originalRequest();
    const query = request.query.bind(request);
    request.query = (statement) => {
      if (statement === OUTLOOK_QUEUE_STATUS_SQL) throw Object.assign(new Error('timeout'), { code: 'ETIMEOUT' });
      return query(statement);
    };
    return request;
  };
  const route = await import('../src/app/api/mergen-rota/reminders/run/route.js');
  const response = await route.GET();
  assert.equal(response.status, 503);
  assert.ok((await response.json()).outlook.reason);
  const { snapshotOperations } = await import('../src/server/observability/telemetryRegistry.js');
  const operation = snapshotOperations().find((row) => row.operation === 'api.reminders.status');
  assert.equal(operation.errorCount, 1);
});

test('uyarı ve telemetri saklama hatası olay saklamasını atlamaz', async (t) => {
  stack(t);
  const pool = await poolApi.getSqlPool();
  let operationalRetention = false;
  const failing = { request() {
    const request = pool.request();
    const query = request.query.bind(request);
    request.query = async (statement) => {
      if (statement.includes('MR_OperationalAlerts') && statement.includes('SELECT')) throw new Error('alerts-failed');
      if (statement.includes('DELETE') && statement.includes('MR_TelemetryOperationSamples')) throw new Error('retention-failed');
      if (statement.includes('DELETE') && statement.includes('MR_OperationalEvents')) operationalRetention = true;
      return query(statement);
    };
    return request;
  } };
  const result = await runTelemetryCycle({ pool: failing, retentionDueAt: 0 });
  assert.equal(result.ok, false);
  assert.equal(operationalRetention, true);
  assert.equal(result.retentionComplete, false);
  assert.ok(result.failures.some((failure) => failure.phase === 'alerts'));
  assert.ok(result.failures.some((failure) => failure.phase === 'telemetry-retention'));
});

...
test('birleştirilen durum yamaları onaylanan kilometre taşı sıfırlamasını SQL kaydına taşır', async () => {
  const stack = await createActualStack(seed({
    IsMilestone: true, PlannedDurationDays: 0, Status: 'done', Progress: 100,
    ActualStart: '2026-09-07', ActualFinish: '2026-09-07'
  }), { sicil: actor, corporateWbsSource: false });
  try {
    const reset = prepareTaskLifecycleIntent(stack.state, taskId, { status: 'todo' }, () => true);
    const first = stack.persistence.updateTask(taskId, reset);
    const second = stack.persistence.updateTask(taskId, { status: 'in_progress' });
    await stack.persistence.flushTaskUpdates([taskId]);
    for (const result of await Promise.all([first, second])) assert.equal(result.ok, true, result.error?.message);
    const row = stack.db.tasks[0];
    assert.equal(row.Status, 'planned');
    assert.equal(row.ActualStart, null);
    assert.equal(row.ActualFinish, null);
...
for (const creator of [false, true]) {
  test(`tekrar yetkisi olmayan kullanıcıya seri kilidi yerine alan yetkisi hatası verilir: oluşturucu ${creator}`, () => {
    const state = {
      currentUser: { id: String(actor) },
      projects: [{ id: projectId, accessLevel: 'PARTIAL' }],
      tasks: [
        { id: taskId, projectId, assigneeIds: [String(actor)], assigneeCount: 2,
          isCurrentUserAssignee: true, isCurrentUserCreator: creator },
        { id: newId, projectId, recurrenceParentId: taskId }
      ]
    };
    const denied = resolveTaskMutationAccess(state, taskId, { recurrence: 'FREQ=WEEKLY' });
    assert.equal(denied.ok, false);
    assert.equal(denied.code, creator ? 'TASK_CREATOR_FIELD_FORBIDDEN' : 'TASK_ASSIGNEE_FIELD_FORBIDDEN');
    state.projects[0].accessLevel = 'FULL';
    assert.equal(resolveTaskMutationAccess(state, taskId, { recurrence: 'FREQ=WEEKLY' }).code, 'TASK_RECURRENCE_LOCKED');
  });
}

...
test('zamanlayıcı ucu kimliksiz erişime açılmaz', () => {
  ...
  // Çağrının otomatik olup olmadığı YALNIZCA zamanlayıcı anahtarından türer…
  assert.match(route, /const automatic = hasReminderSchedulerKey\(request\);/);
  // …ve anahtar yokken yetki bağlamı yüklenip SYSTEM_ADMIN aranır.
  assert.match(route, /if \(!automatic\) \{[\s\S]*?if \(!actor\.isSystemAdmin\)/);
  assert.match(route, /throw new ServerPersistenceError\('FORBIDDEN'/);
});

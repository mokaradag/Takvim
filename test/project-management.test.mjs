...
import { prepareProjectCreation, validateProjectCreationInput } from '../src/state/projectCreation.js';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');

function seed() {
  return {
    calendars: [{ id: 'cal-1', name: 'Standart', workingDays: [1, 2, 3, 4, 5], holidays: [] }],
    projects: [{ id: 'p-1', name: 'Mevcut Proje', color: 'blue', leadId: 'u-1', lead: 'Ayşe', calendarId: 'cal-1', dataDate: '2026-07-22' }],
...
test('project creation validation checks lead, calendar, date, and color', () => {
  const context = seed();
...
test('prepareProjectCreation creates an atomic project and root WBS change set', () => {
  const context = seed();
...
test('project and root WBS persist atomically and survive repository reload', async () => {
...
test('project persistence failure does not partially add the project or root WBS', async () => {
  const repository = createMockRepository(seed(), {
...
test('project creation UI exposes all required fields and is wired to the application action', () => {
  ...
  assert.match(shell, /onCreate=\{addProject\}/);
  assert.match(provider, /prepareProjectCreation/);
  assert.match(provider, /commitChanges\('project\/create'/);
  for (const label of ['Proje adı', 'Proje sorumlusu', 'Proje takvimi', 'Veri tarihi', 'Proje rengi']) {

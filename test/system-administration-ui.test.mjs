...
test('boş metin ölçümü grafikte sıfır veya bağlantı üretmez', (t) => {
  for (const blank of ['', '   ']) {
    const points = [10, blank, 20].map((value, index) => ({ value, bucketStart: new Date(1700000000000 + index * 300000).toISOString() }));
    const view = mountComponent(TrendChart, { series: [{ points }] });
    t.after(() => view.unmount());
    assert.equal(collectSvgNodes(view.output, 'circle').length, 2);
    assert.equal(collectSvgNodes(view.output, 'polyline').length, 0);
    view.render({ series: [{ points: [points[1]] }] });
    assert.match(view.output.props['aria-label'], /veri yok/);
  }
});

test('aynı bileşene yeniden gezinme olay süzgecini tekrar uygular', async (t) => {
  const calls = stubFetch(t, async () => Response.json({ ok: true, events: [], alerts: [], filters: {}, total: 0 }));
  const view = mountComponent(SystemEventsTab, { focusedComponent: 'SMTP', navigationToken: 1 });
  t.after(() => view.unmount());
  await immediate(); view.render();
  findFilterControl(view.output, 'Bileşen', 'select').props.onChange({ target: { value: 'API' } });
  view.render(); await immediate(); view.render();
  assert.match(calls.at(-1), /component=API/);
  view.render({ focusedComponent: 'SMTP', navigationToken: 2 });
  await immediate(); view.render();
  assert.match(calls.at(-1), /component=SMTP/);
});

test('klavye gezinmesi bileşen odağını temizler ve gezinme belirtecini ilerletir', (t) => {
  withDataMode(t, 'demo');
  const prior = globalThis.requestAnimationFrame;
  globalThis.requestAnimationFrame = () => {};
  t.after(() => { globalThis.requestAnimationFrame = prior; });
  const view = mountComponent(SystemAdministrationView, {});
  t.after(() => view.unmount());
  const navigate = () => findElement(view.output, (node) => node.type === SystemHealthStrip).props.onNavigate('olaylar', 'SMTP');
  navigate(); view.render();
  let events = findElement(view.output, (node) => node.type === SystemEventsTab);
  assert.ok(events);
  const token = events.props.navigationToken;
  navigate(); view.render();
  events = findElement(view.output, (node) => node.type === SystemEventsTab);
  assert.ok(events.props.navigationToken > token);
  const selected = () => findElement(view.output, (node) => node.props?.role === 'tab' && node.props['aria-selected']);
  selected().props.onKeyDown({ key: 'ArrowRight', preventDefault() {}, stopPropagation() {} });
  view.render();
  selected().props.onKeyDown({ key: 'ArrowLeft', preventDefault() {}, stopPropagation() {} });
  view.render();
  assert.equal(findElement(view.output, (node) => node.type === SystemEventsTab).props.focusedComponent, null);
});

test('başarısız yeni aralık isteği eski performans verisini göstermez', async (t) => {
  let finish;
  stubFetch(t, async (url) => {
    if (url.includes('range=1h')) return new Promise((resolve) => { finish = resolve; });
    return Response.json({ ok: true, range: { id: '24h' }, operations: [{ operation: 'old-range-marker', count: 3 }], series: [], gauges: {}, summary: {}, resources: {} });
  });
  const view = mountComponent(SystemPerformanceTab, {});
  t.after(() => view.unmount());
  await immediate(); view.render();
  assert.match(JSON.stringify(view.output), /old-range-marker/);
  findButtonWithText(view.output, 'Son 1 saat').props.onClick();
  view.render(); await immediate(); view.render();
  assert.doesNotMatch(JSON.stringify(view.output), /old-range-marker/);
  finish(Response.json({ ok: false, message: 'okunamadı' }, { status: 503 }));
  await immediate(); view.render();
  assert.doesNotMatch(JSON.stringify(view.output), /old-range-marker/);
});

test('kuyruk yaşının kesirli dakika değeri iki görünümde korunur', async (t) => {
  const fixture = overviewFixture({ generatedAt: '2026-09-12T10:00:00Z', outlook: { enabled: true, queue: {}, age: { oldestUnattemptedAt: '2026-09-12T09:59:30Z' } } });
  stubFetch(t, async () => Response.json({ ...fixture, reminders: {}, corporateWbs: {} }));
  for (const [Component, props] of [[SystemOverviewTab, { overview: fixture }], [SystemQueuesTab, { enabled: true }]]) {
    const view = mountComponent(Component, props);
    t.after(() => view.unmount());
    await immediate(); view.render();
    const tile = findElement(view.output, (node) => node.props?.label === 'En eski bekleyen');
    assert.equal(tile.props.value, 0.5);
    assert.equal(tile.props.unit, 'minutes');
  }
});

...
test('doğrulama özeti ve bildirimi üç kalıcı sonucu ayırır', () => {
  ...
  assert.equal(validationNotice('VALID').tone, 'ok');
  assert.match(validationNotice('REJECTED').text, /kurumsal anahtara aktarılmaz/);
  assert.equal(validationNotice('FORBIDDEN').tone, 'warn');
});
...
test('istemci süre sınırı sunucunun sıra ve istek sınırlarından uzundur', () => {
  ...
  // Sunucu sınırları bilinmiyorsa varsayılanlar kullanılır.
  assert.ok(probeTimeoutMs(null) > 20000 + 90000);

...
const { NAV_ITEMS, contentClassName } = await import('../src/components/shell/navigation.js');
...
test('Görev Hareketleri yalnız etkin paneli esnetir ve tek tablo kaydırma alanı kullanır', (t) => {
  ...
  // Kabuk içerik sınıfını gezinme KİMLİĞİNDEN üretir; rapor sayfasının kimliği
  // `rapor`'dur. Bağ kopar da sınıf başka bir adla üretilirse aşağıdaki CSS
  // kuralı hiçbir öğeyle eşleşmez ve tablo boş alan bırakır. Bu yüzden kimlik
  // ve seçici ayrı ayrı değil, kabuğun GERÇEK üreticisiyle birlikte denetlenir.
  const reportsNavItem = NAV_ITEMS.find((item) => item.id === 'rapor');
  assert.ok(reportsNavItem, 'rapor gezinme öğesi bulunmalıdır');
  assert.ok(contentClassName(reportsNavItem.id).split(' ').includes('content-rapor'));
  assert.match(read('src/components/shell/AppShell.jsx'), /className=\{contentClassName\(view,/);

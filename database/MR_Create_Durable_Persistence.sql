SET NOCOUNT ON;
SET XACT_ABORT ON;
/* Filtrelenmiş dizinlerin gerektirdiği oturum seçeneklerinin TAMAMI. */
...
BEGIN TRY
    ...
    /* InstanceId anahtarın PARÇASIDIR: birden çok uygulama süreci aynı kovayı
       yazdığında satırlar birbirinin yüzdeliklerini ezmez ve süreç ölçümleri
       tek bir seriye karışmaz. LastFlushId ise aynı boşaltmanın iki kez
       uygulanmasını engeller: yazma başarılı olup istemci belirsiz bir hata
       aldığında yeniden gönderilen satır sayıları İKİNCİ kez toplamaz. */
    ...
    CREATE TABLE dbo.MR_OperationalEvents (
        ...
        /* Aynı sorunun kararlı parmak izi: bellekteki birleştirme yalnızca tek
           boşaltma turunu kapsar; tur sınırında bölünen yineleme burada da tek
           satırda toplanır. */

...
Reparenting is also available directly on the tree rows. `src/features/wbs/wbsDragPolicy.js` owns the pure drop policy and is independent from the view; it resolves a drop into a concrete move request or an explicit rejection reason.

Three drop positions exist, chosen from the pointer's vertical position inside the target row:

| Position | Pointer band | Result |
| -------- | ------------ | ------ |
| `before` | top 28 %     | node becomes the previous sibling of the target |
...
1. when the parent changes, the existing reparent rules run unchanged (code re-derivation and descendant code rebasing);
2. the node is then placed at the requested index among its siblings and `sortOrder` is renumbered.

...
The merge happens in **both** branches of `schedule()`. It previously ran only when the task had no pending entry: the retained fields were deleted and never merged when a queue entry already existed. That sequence is reachable — a flush starts and awaits the server, the user types again (creating a pending entry while `failed` is still empty), the awaited flush rejects and writes the retained fields, and the next keystroke dropped them.
...
Gerçek Sistem'de aynı mekanizma görünür sekmede otomatik olarak da çağrılır. Dağıtım aralığı `NEXT_PUBLIC_MERGEN_ROTA_AUTO_REFRESH_INTERVAL_MS` ile belirlenir (varsayılan `60000`, alt sınır `30000` milisaniye; geçersiz değer varsayılana döner). Kendini zamanlayan tek-uçuşlu denetleyici sekme gizlenince zamanlayıcıyı kaldırır; görünür olduğunda son başarılı snapshot aralık kadar eskiyse hemen, değilse kalan süre sonunda çalışır. Demo repository'si için denetleyici kurulmaz.

Otomatik çağrı `discardFailedTaskUpdates` seçeneğini hiçbir zaman kullanmaz. Bekleyen yazmalar normal sırayla tamamlanır; reddedilmiş yerel Task yaması varsa güvenlik denetimi snapshot yükünü atlar ve sonraki aralıkta yeniden değerlendirir. Atlanan ya da başarısız deneme tam aralık beklemeden tekrar edilmez. Geçici arka plan yükleme hatası mevcut başarılı snapshot'ı hata durumuna çevirmez; `SESSION_REQUIRED`, `UNAUTHORIZED` ve `FORBIDDEN` ise mevcut kimlik/yetki davranışına aktarılır. Manuel yenileme aynı giriş noktasında kalır ve ortak etkin-yük denetimi iki yenilemenin yarışmasını engeller. Otomatik yük sürerken gelen manuel istek onun sessiz arka plan sonucunu paylaşmaz; otomatik turun arkasına tek kez alınır ve kendi görünür yükleme/hata yaşam döngüsünü çalıştırır.

A manual reload does **not** tear down the application shell. `dataStatus` returns to `loading`, but `hasLoadedOnce` stays true after the first successful load, and `AppDataBoundary` therefore keeps the shell mounted and shows the blocking screen only for the initial load. Automatic reload does not set the visible loading state at all once data exists. Before these rules, every post-save refresh remounted `AppShell`, discarding view state and re-showing the welcome screen as if the application had just started. A failed manual refresh with data already present is reported in the persistence status area instead of replacing the application.

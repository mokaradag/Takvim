...
`ai/` is the AI subsystem (see [AI platform](AI-PLATFORM.md)). It is deliberately a leaf: ordinary request paths (snapshot, commit, task editing, reports) never import it, and only the settings card, the System Management health/integration probes and the telemetry cycle's AI load sampling reach it. `aiGateway.js` is its single execution path — trusted Sicil → corporate directory membership (rehber üyeliği; bounded pre-admission SQL check with its own deadline) → configuration → profile → capacity lease (`admissionController.js`) → credential (`aiCredentialService.js`, encrypted by `credentialVault.js`) → OpenAI-compatible provider (`providers/`); the credential read and the provider call share one deadline/cancellation signal (`aiDeadline.js`). Business code asks for a capability profile, never a concrete model; the profile → model mapping is data (`defaultModelRegistry.js` or an operator-supplied JSON file). The versioned snapshot of the current on-premises catalogue is `config/ai-model-registry.onprem.json`; production points `MERGEN_ROTA_AI_MODEL_REGISTRY_PATH` at its deployed absolute-path copy. The gateway refuses to run inside an open SQL transaction, and no SQL connection or transaction is held while a model generates. The provider never receives SQL credentials and there is no generic SQL execution surface.
...
- AI configuration, personal-key storage, capacity control, provider adapters and the model/capability registry: `src/server/ai`, with pure shared rules in `src/domain/ai` (see `docs/AI-PLATFORM.md`)
...
### Outlook product and lifecycle boundary

Rota remains date-granularity task management. Outlook reuses `taskCalendarDate()` for one all-day, TRANSPARENT/FREE deadline marker with exclusive next-day DTEND; no time slots or start–finish span is exported. SMTP/iCalendar is a one-way projection: no mailbox reads, reverse synchronization, Graph/Entra/EWS, Outlook Tasks/To Do or Outlook recurring master is introduced. Generated task occurrences retain independent TaskId/UserSicil UIDs. Changes to Rota-managed content may overwrite manual Outlook edits; subscription activity and SMTP delivery never confirm current calendar contents.

Migration 0011 adds completion suspension/date, cancellation cause, last delivered method and reserved date to the existing durable row. Today/future completion cancels the marker while preserving opt-in identity; historical completion settles silently. Reopening requests the current canonical representation with the same UID and a higher revision. Explicit removal, deletion and visibility loss close the preference and never auto-reactivate it. Completion intent is persisted in the task transaction; QueueSeq and lease conditions preserve later writes during in-flight delivery. SMTP is required for Outlook or any e-mail-reminder use (`SMTP_HOST`, valid `SMTP_FROM` and corporate mail rules); only the reminder scheduler is optional for Outlook.

The shared DateRangeFilter renders the existing compact clear action for Özet and performance reports. It resets date state and shared organization hierarchy together and removes derived organizationTaskIds. Activity reports reset their own query, dependent fields and pagination. All filtering starts from authorized workspace data or the authorized report endpoint; clearing never changes session authorization or workspace selection.

...
export function normalizeTaskAssigneePatch(patch = {}, people = []) {
  if (hasOwn(patch, 'assigneeIds')) {
    if (!Array.isArray(patch.assigneeIds)) return issue('TASK_ASSIGNEES_INVALID', 'Görev sorumluları dizi olmalıdır.');
    const { sorumlu: _names, ...canonical } = patch;
    return { ok: true, patch: {
      ...canonical,
      assigneeIds: [...new Set(patch.assigneeIds.map((id) => String(id).trim()).filter(Boolean))],

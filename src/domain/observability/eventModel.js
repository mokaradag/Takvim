...
export const EVENT_CODES = Object.freeze({
  ...
  // Ölçümün YOKLUĞU, başarısızlıktan ayrı bir koşuldur: durum okunamadığında
  // "eşitleme başarısız" demek, gerçekleşmemiş bir arızayı bildirmek olurdu.
  CORPORATE_WBS_STATUS_UNAVAILABLE: {
    severity: EVENT_SEVERITIES.WARNING,
    component: COMPONENTS.CORPORATE_WBS,
    summary: 'CN43N eşitleme durumu ölçülemiyor',
    action: 'Kurumsal WBS kaynağının erişilebilirliğini denetleyin; henüz hiç eşitleme yapılmamış olabilir.'
  },
  SMTP_NOT_CONFIGURED: {
    severity: EVENT_SEVERITIES.WARNING,
  ...
  TELEMETRY_DATA_DROPPED: {
    severity: EVENT_SEVERITIES.WARNING,
    component: COMPONENTS.APPLICATION,
    summary: 'Telemetrinin bir bölümü sınır aşımı nedeniyle kaydedilemedi',
    action: 'Kalıcılaştırma kesintisini ve ölçüm hacmini inceleyin; geçmişte boşluk olabilir.'
  },
  ADMIN_ACTION_FAILED: {
    severity: EVENT_SEVERITIES.ERROR,
    component: COMPONENTS.APPLICATION,
    summary: 'Yönetici eylemi başarısız.',
    action: 'Eylem sonucunu ve ilgili bileşenin durumunu inceleyin.'
  },
...
export function componentLabel(value) {
  ...
  return Object.hasOwn(COMPONENT_LABELS, key) ? COMPONENT_LABELS[key] : key;
}

export function componentTab(value) {
  const key = String(value ?? '');
  return Object.hasOwn(COMPONENT_TABS, key) ? COMPONENT_TABS[key] : ADMIN_TABS.OVERVIEW;
}

/** Kod kataloğundaki tanım; bilinmeyen kod güvenli bir yedeğe düşer. */
export function eventDefinition(code) {
  const key = String(code ?? '');
  return Object.hasOwn(EVENT_CODES, key) ? EVENT_CODES[key] : EVENT_CODES.OPERATION_FAILED;
}

...
function sanitizeContextValue(value, depth) {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    const baseKey = sanitizeText(key).slice(0, MAX_KEY_LENGTH) || REDACTED;
    let safeKey = baseKey;
    for (let suffix = 2; Object.hasOwn(output, safeKey); suffix += 1) {
      const tail = `_${suffix}`;
      safeKey = `${baseKey.slice(0, MAX_KEY_LENGTH - tail.length)}${tail}`;
    }

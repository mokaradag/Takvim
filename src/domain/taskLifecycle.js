...
export function normalizeTaskLifecycle(task = {}, patch = {}, today) {
  // İstemcide başlık/etiket gibi yaşam döngüsü dışı yamalar aynen kalır.
  // SQL sınırı ise `resetActualDates` alanını açıkça (false olsa bile) taşır;
  // bu, dar satır güncellemelerinde kilometre taşı değişmezlerinin yeniden
  // kanonikleştirilmesi gerektiğini bildirir.
  const lifecycleIntent = fields.some((field) => owns(patch, field)) || owns(patch, 'resetActualDates');
  if (!lifecycleIntent) return patch;
  ...
  // Kilometre taşı yaşam döngüsü sunucu yazmalarında kanonik kalır. Normal
  // görev → kilometre taşı geçişinde eski ilerleme ve gerçekleşen tarihler
  // taşınmaz; istemcideki ilgisiz yamalar ise yukarıdaki sınırda aynen döner.
  const milestone = milestoneCompletion(task, patch, today);

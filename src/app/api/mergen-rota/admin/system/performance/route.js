...
const GAUGE_SERIES_KEYS = [
  ...
  GAUGE_KEYS.AI_ACTIVE_REQUESTS,
  GAUGE_KEYS.AI_QUEUED_REQUESTS
];
...
export const GET = withRouteObservability('api.admin.system.performance', async (request) => {
  try {
    ...
    // Temel pencere, seçilen aralığın BAŞLADIĞI anda biter. Aksi halde `7g`
    // seçiminde güncel pencere ile taban aynı kayıtları içerir (karşılaştırma
    // hep düz çıkar), kısa aralıklar ise kendi tabanlarının içinde kalıp
    // değişimi seyreltirdi.
    const baselineUntil = since;
    const baselineSince = new Date(baselineUntil.getTime() - BASELINE_WINDOW_MS);
    ...
    return Response.json({
      ...
      baseline: {
        ...
        since: baselineSince.toISOString(),
        until: baselineUntil.toISOString(),
        // Örnek eşiği İKİ pencere için de uygulanır: az örnekli bir güncel
        // pencereden çıkarılan yüzde, kutucukta gerçek bir değişim gibi görünür.
        p95: compareToBaseline(summary.p95Ms, baseline.p95Ms, {
          baselineSamples: baseline.count,
          currentSamples: summary.count
        }),
        errorRate: compareToBaseline(summary.errorRate, baseline.errorRate, {
          baselineSamples: baseline.count,
          currentSamples: summary.count
        }),

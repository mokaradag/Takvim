...
export const POST = withRouteObservability('api.admin.system.events.acknowledge', async (request) => {
  try {
    ...
    // Şema eksikken onay hiç DENENMEMİŞTİR. Bunu "zaten onaylanmış" diye
    // bildirmek, yöneticiye uyarının durumu hakkında yanlış bilgi verirdi.
    if (!outcome.schemaReady) {
      throw new ServerPersistenceError(
        'DATABASE_UNAVAILABLE',
        'Gözlemlenebilirlik şeması henüz hazır değil; uyarı onaylanamadı.'
      );
    }
    return Response.json({
      ok: true,

...
const DERIVED_LIFECYCLE_FIELDS = Object.freeze(['actualStart', 'actualFinish', 'progress']);
...
export function createStateMutationOrchestrator({
  ...
  const persistAction = (operation, actionOrFactory, options = {}) => queue.enqueue(async () => {
    ...
    let planned;
    try {
      planned = appStateReducer(before, action);
    } catch (error) {
      ...
      return {
        ...
        error: {
          kind: 'domain',
          code: error.code,
          message: error.message || 'Değişiklik doğrulanamadı.',
          operation,
          details: null
        }
      };

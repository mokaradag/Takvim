...
const MESSAGES = Object.freeze({
  ...
  TASK_COMPLETED_ADD_BLOCKED: 'Tamamlanmış görev Outlook takvimine eklenemez.',
...
async function addOne(executor, { taskId, sicil, link, send, now, queueOnly = false }) {
  ...
  if (taskState.task.status === 'done') return { status: 'FAILED', code: 'TASK_COMPLETED', message: outlookMessage('TASK_COMPLETED_ADD_BLOCKED') };
  if (!payload.ok) {
 ...
 * bağımsız iCalendar davetini alır ve tek bir görevin hatası ötekilerini

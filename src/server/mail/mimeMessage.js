 ...
 *
 * `calendar` verildiğinde yapı Outlook/Exchange'in beklediği biçimdedir:
 *
 ...
 *
 * Davetin İKİNCİ bir kopyası EK OLARAK TAŞINMAZ. Aynı takvim içeriği hem
 * alternatif parçada hem de `application/ics` ekinde gönderildiğinde Outlook
 * iletiyi takvim iletisi olarak işlemek yerine iki dosya eki gösteriyordu
 * ("not supported calendar message.ics" ve "mergen-rota.ics"). Tek alternatif
 * parça, iptal ve davet iletilerinin doğrudan takvim öğesi olarak açılmasını
 * sağlar. `calendar` verilmediğinde gövde eskisi gibi yalın kalır —
 * hatırlatma e-postaları bu yüzden değişmez.
 *
 * @param {{from:string, fromName?:string, to:string[], subject:string,
 *   html:string, text:string, messageId?:string, date?:Date,
 *   calendar?:{method:'REQUEST'|'CANCEL', content:string}|null}} input

...
import { trustsInboundCorrelationHeader } from './observabilityConfig.js';
...
export function withRouteObservability(operation, handler, { component = COMPONENTS.API } = {}) {
  return async function observedRouteHandler(request, ...rest) {
    // İlişkilendirme kimliği SUNUCUDA üretilir. İstemci başlığı yalnızca onu
    // her istekte yeniden yazan güvenilir bir vekil sunucunun arkasında,
    // açıkça açıldığında kabul edilir; aksi halde bir istemci kendi kimliğini
    // dayatıp başkasının iziyle aynı gruba düşebilirdi.
    const inbound = trustsInboundCorrelationHeader()
      ? acceptedCorrelationId(request?.headers?.get?.(CORRELATION_HEADER))
      : null;

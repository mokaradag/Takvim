  ...
  if (apiSummary && Number(apiSummary.count || 0) >= MIN_SAMPLES_FOR_RATE_ALERT) {
    ...
    if (Number.isFinite(errorRate) && errorRate > Number(thresholds.errorRate ?? 0.05)) {
      add(condition(COMPONENTS.API, 'API_ERROR_RATE_HIGH', EVENT_SEVERITIES.ERROR,
        ...
        { scope: applicationInstanceId(), context: { count: Number(apiSummary.count), errorCount: Number(apiSummary.errorCount || 0) } }));
    }
...
const COMPONENT_ALERT_CODES = Object.freeze({
  ...
  [COMPONENTS.CORPORATE_WBS]: ['CORPORATE_WBS_STALE', 'CORPORATE_WBS_STATUS_UNAVAILABLE'],
...
export function deriveUnmeasuredAlertKeys({ components = [], apiSummary = null } = {}) {
  ...
  if (!apiSummary || !Number.isFinite(count) || count < MIN_SAMPLES_FOR_RATE_ALERT) {
    keys.add(alertKey(COMPONENTS.API, 'API_ERROR_RATE_HIGH', applicationInstanceId()));
    keys.add(alertKey(COMPONENTS.API, 'API_LATENCY_HIGH', applicationInstanceId()));
...
export function evaluateAlertTransitions(conditions = [], {
  ...
  const ownsKey = (key) => {
    ...
    return !['MEMORY_PRESSURE', 'API_ERROR_RATE_HIGH', 'API_LATENCY_HIGH'].includes(parsed.code)
      || parsed.scope === applicationInstanceId();
  };

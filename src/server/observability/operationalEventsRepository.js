...
import {
  OPERATIONAL_ACTIVE_ALERT_KEYS_SQL,
...
/**
 * Arama metnini `LIKE` kalıbına çevirir.
 *
 * Metakarakterler ATILMAZ, KAÇIRILIR: `_` işaretini boşlukla değiştirmek
 * `HTTP_500` ya da `TELEMETRY_DATA_DROPPED` aramasını başka bir dizgeye çevirir
 * ve yönetici eşleşen kaydı hiç göremezdi. Sorgular bu kalıbı `ESCAPE '\'` ile
 * okur (bkz. telemetryQueries.js).
 */
export function likeSearchPattern(value) {
...
/**
 * Yinelenen olayların TOPLANDIĞI zaman penceresi.
 *
 * Bellekteki birleştirme yalnızca tek bir boşaltma turunu kapsar; tur sınırında
 * bölünen aynı sorun kalıcı tarafta da tek satırda toplanmalıdır. Pencere
 * SINIRLIDIR: iki gün arayla yaşanan iki kesinti geçmişte ayrı durmalıdır.
 */
export const EVENT_AGGREGATION_WINDOW_MS = 15 * 60 * 1000;

/**
 * Aynı sorunun kararlı parmak izi.
 *
 * Özet 300 karaktere kadar çıkabilir; anahtar sütunu sınırlı olduğu için özet
 * kısa ve kararlı bir özüte indirgenir. Anahtar bellekteki birleştirme
 * anahtarıyla AYNI alanlardan türer.
 */
function aggregationKey(event) {
  const digest = createHash('sha1').update(String(event.summary ?? '')).digest('hex').slice(0, 16);
  return `${event.severity}:${event.component}:${event.code}:${digest}`.slice(0, 200);
}
...
/**
 * Bağlamı SINIRLI ama GEÇERLİ JSON olarak serileştirir.
 *
 * Metni kesmek, JSON'u bir simgenin ortasından bölüp çözümlenemez hale
 * getirirdi: bütün tanı bağlamı sessizce kaybolurdu. Bunun yerine sınır
 * aşıldığında alan ATILIR ve ne kadar alanın atıldığı bağlamda bildirilir.
 */
function contextJson(context) {
  ...
  try {
    ...
    if (full.length <= MAX_CONTEXT_LENGTH) return full;
    ...
    for (const [key, value] of Object.entries(safe)) {
      const candidate = { ...bounded, [key]: value, truncated: true, droppedKeys: dropped + 1 };
    ...
    if (!dropped) return JSON.stringify(bounded).slice(0, MAX_CONTEXT_LENGTH);
    const result = JSON.stringify({ ...bounded, truncated: true, droppedKeys: dropped });
    return result.length <= MAX_CONTEXT_LENGTH ? result : JSON.stringify({ truncated: true, droppedKeys: dropped });
  } catch {
    return null;
  }
}
...
export function queueOperationalEvent({
  ...
  const accepted = store.pending.length < MAX_PENDING_EVENTS;
  if (!accepted) {
    store.dropped += 1;
  } else {
...
export function drainOperationalEvents() {
  ...
  for (const event of pending) {
    ...
    existing.occurrenceCount += eventOccurrences(event);
    // Son gözlem anı ve son ilişkilendirme kimliği korunur: yönetici en taze
    // örneğin izine ulaşabilmelidir. Bağlam da AYNI örnekten gelmelidir;
    // ilkinin bağlamıyla sonuncunun ilişkilendirme kimliğini bir arada
    // göstermek, birbirine ait olmayan tanı alanları üretirdi.
    ...
    existing.contextJson = event.contextJson || existing.contextJson;
  }
...
export function restoreDroppedEventCount(count) {
  if (Number.isFinite(count) && count > 0) buffer().dropped += Math.trunc(count);
}

/**
 * Yazılamayan olayları kuyruğun BAŞINA geri koyar.
 *
 * Veritabanı kesintisi, kesinti sonrasında gereken olay kayıtlarını yok
 * etmemelidir. Kuyruk sınırı yine korunur; taşan kayıtlar düşürülen sayacına
 * yazılır ve yönetim konsoluna bildirilir.
 */
export function restoreOperationalEvents(events = []) {
  const store = buffer();
  const restored = [];
  for (const event of events) {
    if (restored.length + store.pending.length >= MAX_PENDING_EVENTS) {
      store.dropped += eventOccurrences(event);
      continue;
    }
    restored.push(event);
  }
  store.pending = [...restored, ...store.pending];
...
/**
 * Bekleyen olayları yazar.
 *
 * Hata FIRLATMAZ: yazılmayan olaylar tam olarak bildirilir ki çağıran onları
 * kuyrukta tutabilsin. Yazılmış bir olayı yeniden göndermek yinelenen satır
 * üretirdi.
 *
 * @returns {Promise<{written: number, schemaReady: boolean, pending: Array, error: Error|null}>}
 */
export async function persistOperationalEvents(executor, events = []) {
  ...
  try {
    for (const event of events) {
      ...
      request.input('occurredAt', sql.DateTime2, occurredAt);
      request.input('aggregationKey', sql.VarChar(200), aggregationKey(event));
      request.input('aggregateSince', sql.DateTime2, new Date(occurredAt.getTime() - EVENT_AGGREGATION_WINDOW_MS));
    ...
    return { written, schemaReady: true, pending: [], error: null };
  } catch (error) {
    const pending = events.slice(written);
...
export async function loadOperationalEvents(executor, {
  ...
  try {
    ...
    request.input('search', sql.NVarChar(200), likeSearchPattern(search));
    request.input('offset', sql.Int, safeOffset);
    request.input('limit', sql.Int, safeLimit);
    const result = await request.query(OPERATIONAL_EVENT_PAGE_SQL);
    const rows = result.recordset || [];
    // Toplam sayı sayfadan bağımsızdır: boş sayfa (sapma eşleşen satır sayısını
    // aştığında) tek bir satır döner ve o satırda yalnızca toplam bulunur.
    const events = rows.filter((row) => row.OperationalEventId != null);
    return {
      schemaReady: true,
 ...
 *
 * Süreç yeniden başladığında çırpınma izleyicisi boştur; bu liste izleyiciye
 * tohumlanmazsa, yeniden başlatma sırasında koşulu düzelen bir uyarı sonsuza
 * dek açık kalırdı.
 */
export async function loadActiveAlertKeys(executor) {
  try {
    const result = await executor.request().query(OPERATIONAL_ACTIVE_ALERT_KEYS_SQL);
    return {
      schemaReady: true,
      keys: (result.recordset || []).map((row) => String(row.AlertKey || '')).filter(Boolean)
    };
  } catch (error) {
    if (isMissingTelemetrySchema(error)) return { schemaReady: false, keys: [] };
    throw error;
  }
}

export async function loadOperationalAlerts(executor, { activeOnly = true, component = null, severity = null, code = null, search = null, since, limit = 50 } = {}) {
  try {
    const request = executor.request();

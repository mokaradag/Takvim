...
/** Yalnızca zamanlanmış örnekleyici ölçüm penceresini kapatır. */
export function resetEventLoopSample() {
  try { globalThis[SAMPLE_KEY]?.loopMonitor?.reset(); } catch { /* ölçüm kullanılamıyor */ }
}
...
export async function startResourceMonitoring() {
  ...
  globalThis[SAMPLE_KEY] = { loopMonitor: null, lastCpu: null, lastCpuPercent: null };
  try {
 ...
 */
function processCpuPercent(now, { sample = false } = {}) {
  const store = globalThis[SAMPLE_KEY];
  if (!store || typeof process.cpuUsage !== 'function') return gauge(null, { unit: '%', available: false });
  const unavailable = gauge(null, { unit: '%', available: false });
  // Temel okumayı YALNIZCA zamanlanmış telemetri örneklemesi ilerletir. Sağlık
  // yoklaması ya da yönetim isteği araya girip temeli tazelerse, bir sonraki
  // zamanlanmış ölçüm ilgisiz ve çok kısa bir aralığı bildirirdi: değer isteğin
  // zamanlamasına göre değişirdi. Öteki okuyucular son hesaplanan oranı görür.
  if (!sample) return store.lastCpuPercent ?? unavailable;
  const usage = process.cpuUsage();
  const previous = store.lastCpu;
  store.lastCpu = { at: now, user: usage.user, system: usage.system };
  if (!previous) {
    store.lastCpuPercent = unavailable;
    return store.lastCpuPercent;
  }
  const elapsedMs = now - previous.at;
  if (elapsedMs <= 0) return store.lastCpuPercent ?? unavailable;
  const usedMs = ((usage.user - previous.user) + (usage.system - previous.system)) / 1000;
  store.lastCpuPercent = gauge(Math.max(0, Math.min(100, (usedMs / elapsedMs) * 100)), { unit: '%' });
  return store.lastCpuPercent;
}
 ...
 *
 * `sampleCpu` YALNIZCA zamanlanmış telemetri örneklemesinde `true` geçilir;
 * CPU temel okumasının sahibi odur (bkz. `processCpuPercent`).
 */
export function readResourceMetrics(now = Date.now(), { sampleCpu = false } = {}) {
  ...
  return {
    ...
    cpuPercent: processCpuPercent(now, { sample: sampleCpu }),

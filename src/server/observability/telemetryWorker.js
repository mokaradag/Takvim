...
export async function runTelemetryCycle({
  ...
  // Yazma başarısızlığı turu düşürür; ama bekleyen veri yukarıda güvenceye
  // alındıktan sonra.
  ...
  if (now >= retentionDueAt) {
    ...
    for (let batch = 0; batch < MAX_RETENTION_BATCHES; batch += 1) {
      const telemetry = await applyTelemetryRetention(executor, { cutoff, batchSize: RETENTION_BATCH_SIZE });
      const operational = await applyOperationalRetention(executor, { cutoff, batchSize: RETENTION_BATCH_SIZE });

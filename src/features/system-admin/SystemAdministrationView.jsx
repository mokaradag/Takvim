...
export function SystemAdministrationView() {
  ...
  const [activeTab, setActiveTab] = useState(DEFAULT_SYSTEM_ADMIN_TAB);
  ...
  useEffect(() => {
    setActiveTab(readStoredSystemAdminTab(browserStorage()));
    setTabRestored(true);
  }, []);

  useEffect(() => {
    if (tabRestored) writeStoredSystemAdminTab(browserStorage(), activeTab);
  ...
  return (
    <div className="sysadmin-page col" style={{ gap: 14 }}>
      ...
      <SystemHealthStrip
        ...
        // Demo Kipinde üretim ucu HİÇ çağrılmaz: yenileme düğmesi `run`'a
        // doğrudan bağlanırsa kaynağın `enabled` bayrağını atlayıp canlı
        // istek gönderirdi.
        onRefresh={actualMode ? overview.refresh : undefined}
        onNavigate={goToTab}
      />

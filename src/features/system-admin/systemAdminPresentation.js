...
import { formatBytes, formatDuration, formatMinutes, formatPercent, formatRelativeTime } from '../../domain/observability/metrics.js';
...
export function healthNarrative({ state, counts = {}, attentionCount = 0 } = {}) {
  ...
  // Yapılandırılmamış gözlemlenebilirlik SAĞLIK DEĞİLDİR: sağlıklı bileşen
  // oranı bildirmek, ölçüm yokken sistemi ayakta göstermek olurdu.
  if (normalized === HEALTH_STATES.NOT_CONFIGURED) {
    return 'Sistem gözlemlenebilirliği yapılandırılmadı; sağlık ölçülemiyor.';
  }
...
export function formatMetricValue(value, unit) {
  ...
  // DAKİKA ölçüsü sayım gibi yuvarlanmaz: kesirli bekleme süresi hem değerini
  // hem birimini yitirirdi.
...
export function baselineNarrative(comparison, { unit = 'ms' } = {}) {
  ...
  const base = `7 günlük baz: ${formatMetricValue(comparison.baseline, unit)}`;
  // Sıfır tabanda oran tanımsızdır; "%Infinity" yerine yön sözcükle bildirilir.
  if (comparison.changeRatio == null || !Number.isFinite(Number(comparison.changeRatio))) {
    return comparison.direction === 'up' ? `${base} · yeni oluştu` : base;
  }

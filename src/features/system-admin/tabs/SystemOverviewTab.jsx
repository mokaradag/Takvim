...
export function SystemOverviewTab({
  ...
  const [selection, setDetail] = useState(null);
  const selectedItem = selection?.kind === 'component'
    ? overview?.components?.find((item) => item.key === selection.id)
    : overview?.attention?.find((item) => item.id === selection?.id);
  const detail = selection && selectedItem ? { kind: selection.kind, item: selectedItem } : null;
  useEffect(() => {
    if (selection && !selectedItem) setDetail(null);
  ...
  // Hiç veri alınamadıysa bölümler BOŞ değil, ALINAMADI durumundadır: "Açık bir
  // sorun yok" demek, ölçülemeyen bir sistemi sağlıklı göstermek olurdu.
  const requestError = !overview && error
    ? `Sistem durumu alınamadı (${error.code}). ${error.message || ''}`.trim()
    : null;
  // Kuyruk yaşı SUNUCU anlık görüntü anına göre hesaplanır; tarayıcı saatiyle
  // sunucu arasındaki kayma yanlış bir yaş (ve yanlış uyarı tonu) üretirdi.
  const generatedAtMs = overview?.generatedAt ? new Date(overview.generatedAt).getTime() : null;
  const oldestUnattemptedAt = overview?.outlook?.age?.oldestUnattemptedAt || null;
  const oldestUnattemptedMs = oldestUnattemptedAt ? new Date(oldestUnattemptedAt).getTime() : null;
  const oldestMinutes = Number.isFinite(generatedAtMs) && Number.isFinite(oldestUnattemptedMs)
    ? Math.max(0, Math.round((generatedAtMs - oldestUnattemptedMs) / 60000))
    : null;
  ...
  return (
    <div className="col sysadmin-overview" style={{ gap: 14 }}>
        ...
        {error && (
          <p className="sysadmin-inline-error" role="status">
            ...
            {overview
              ...
              : `Sistem durumu alınamadı (${error.code}); henüz geçerli ölçüm yok.`}
          </p>
        )}
      </section>
      ...
      <AdminSection
        ...
        error={requestError}
        empty={!requestError && attention.length === 0}
          ...
          {attention.map((item) => (
            <li key={item.id} className={`sysadmin-attention-item sysadmin-attention-${severityTone(item.severity)}`}>
              ...
              <div className="sysadmin-attention-links">
                <button type="button" className="btn ghost sm" onClick={() => setDetail({ kind: 'attention', id: item.id })}>
                  Ayrıntı
                </button>
      ...
      <AdminSection
        ...
        error={requestError}
        empty={!requestError && components.length === 0}
        ...
        <div className="sysadmin-component-grid">
          {components.map((component) => (
            <button
              ...
              onClick={() => setDetail({ kind: 'component', id: component.key })}
            >
      ...
      >
        <div className="sysadmin-tile-grid">
          ...
          {/* Ölçüm YOKLUĞU "hata yok" değildir: telemetri gelmediğinde
              yönetici sıfır hata gördüğünü sanmamalıdır. */}
          <MetricTile label="Hata oranı" value={api.errorRate} unit="ratio" icon="Alert" trend={trends.errorRate}
            trendColor="var(--status-overdue)"
            hint={api.errorCount == null ? 'ölçüm yok' : (api.errorCount ? `${api.errorCount} hata` : 'hata yok')} />
          ...
          {/* Bekleyen kayıt yoksa yaş SIFIR değil, ÖLÇÜLEMEZDİR. */}
          <MetricTile label="En eski bekleyen" icon="Clock" unit="count" value={oldestMinutes}
      ...
      <AdminSection
        ...
        error={requestError}
        empty={!requestError && (overview?.feed || []).length === 0}
      ...
      {detail && (
        <DetailDrawer
          ...
          subtitle={detail.kind === 'component' ? healthLabel(detail.item.state) : severityLabel(detail.item.severity)}
        ...
        >
          {detail.kind === 'component' ? (
            <div className="col" style={{ gap: 10 }}>
              ...
              <DrawerField label="Durum" value={healthLabel(detail.item.state)} />

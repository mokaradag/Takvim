  ...
  return (
    <div className="col" style={{ gap: 14 }}>
      <div className="sysadmin-toolbar">
        <span className="sysadmin-toolbar-title" role="status" aria-live="polite">
          {resource.data?.generatedAt ? `Durum ${formatRelativeTime(resource.data.generatedAt)} alındı.` : resource.error ? 'Bağımlılık durumu alınamadı.' : 'Bağımlılık durumu yükleniyor…'}
        </span>
      ...
      <AdminSection
        ...
        error={!resource.data && resource.error ? `Entegrasyon bilgisi alınamadı (${resource.error.code}).` : null}
        empty={!resource.loading && !resource.error && integrations.length === 0}
              ...
              <li key={integration.id} className="sysadmin-integration">
                ...
                <div className="sysadmin-integration-actions">
                  {integration.testable && (
                    <button type="button" className="btn sm" disabled={testing != null} onClick={() => runTest(integration.id)}>
